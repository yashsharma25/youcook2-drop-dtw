...
from data.youcook2_loader import YouCook2DatasetLoader
...
class DataModule(pl.LightningDataModule):
    def __init__(self, dataset_name, n_cls, batch_size):
        ...
        # self.lmdb_path = os.path.join(folder, 'dat')
        self.n_cls = n_cls
        ...
        self.train_dataset = YouCook2DatasetLoader(split_type='train')
        self.val_dataset = YouCook2DatasetLoader(split_type='val')
        self.test_dataset = YouCook2DatasetLoader(split_type='test')

        print(len(self.train_dataset), len(self.val_dataset))

    def train_dataloader(self):
        #batch_idx_sampler = BatchIdxSampler_Class(self.train_dataset, 32, self.batch_size)
        ...
        train_loader = DataLoader(self.train_dataset,  batch_size = 4)
        print("Train loader size", len(train_loader))
        print()
        return train_loader

    def val_dataloader(self):
        val_loader = DataLoader(self.val_dataset, collate_fn=flatten_batch)
        print("Val data loader len", len(val_loader))
        return val_loader
